...
To import DVH data into \textsf{R}, it should be exported as a plain text file from Varian Eclipse$^{\mathrm{TM}}$ (Versions 10--13), from CadPlan$^{\mathrm{TM}}$, Pinnacle$^{3\,\mathrm{TM}}$ (version 9), or from Oncentra MasterPlan$^{\mathrm{TM}}$ (version 4.3).\footnote{Pinnacle$^{3}$ files have to be exported such that information from one patient is contained in one directory. The directory layout and required files are explained in \texttt{help(readDVH)}.} Cumulative and differential DVHs are supported, as are sum plans. The measurement unit for absolute dose can be Gy or cGy, the measurement unit for volume has to be cm$^{3}$. DVH text files can be read with \texttt{readDVH()}.
...
\item \texttt{"DEUD"}: The generalized equivalent uniform dose (gEUD, \citeNP{Niemierko1999,Wu2002}). This can be based on EQD2 values if information on the fractionation is provided as well.
\item \texttt{"DNTCP"}: The normal tissue complication probability (NTCP) according to the Lyman \citeyear{Lyman1985} probit model, the Niemierko \citeyear{Niemierko1999} logit model, or the K\"{a}llman \citeyear{Kallman1992} Poisson model. This can be based on EQD2 values if information on the fractionation is provided as well \cite{IAEA2008}.
\item \texttt{"DTCP"}: The tumor control probability (TCP) according to the Lyman probit model, the Niemierko logit model, or the K\"{a}llman Poisson model. This can be based on EQD2 values if information on the fractionation is provided as well.
\end{itemize}
...
For calculating the minimal Euclidean distance between the constraint point and the DVH, the constraint point is orthogonally projected onto each DVH segment between (interpolated) DVH nodes. The relative Euclidean distance is this minimal distance divided by the distance of the constraint point to the closer one of both axes (dose and volume).
...
The linear-quadratic model gives the following formula for the biologically effective dose (BED) with total dose $D$, fraction dose $d$ and a tissue-dependent $\alpha / \beta$ ratio \cite{IAEA2008}.

$$
\mathrm{BED} = D \left[1 + \frac{d}{\alpha / \beta}\right]
$$

The isoeffective dose for two different fractionation schemes can be calculated from solving this equation for the desired measure:
...
$$
\mathrm{EQD}_{2} = D_{1} \frac{d_{1} + (\alpha / \beta)}{2 + (\alpha / \beta)} = \frac{\mathrm{BED}}{1 + \frac{2}{\alpha / \beta}}
$$
